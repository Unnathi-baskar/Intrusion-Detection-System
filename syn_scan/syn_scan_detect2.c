...
void add_log(const char *line) {
	FILE *fp = fopen("syn_scan_logs.txt", "a");
	if(fp){
	fprintf(fp, "%s\n", line);
	fclose(fp);
	}	
 ...
 char *copy = strdup(line);
    ...
    // if client connected, send it now
    if (ssl) {
...
int main(int argc, char **argv) {
    if (argc < 5 || argc > 6) {
        fprintf(stderr,
          "Usage: sudo %s <iface> <target-IP> <cert.pem> <key.pem> [port]\n", argv[0]);
        exit(1);
    }
    iface    = argv[1];
    target_ip= argv[2];
    ...
    // main loop: sniff + accept client
    while (1) {
        // try to accept a new client (once)
        if (client_fd < 0) {
